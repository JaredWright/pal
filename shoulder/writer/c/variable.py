...
def return_variable(outfile: TextIO, var: str, indent: int=0):
    """Writes a return statement using the given variable

    Args:
        outfile: The output file to be written
        var: The name of the variable to be returned
        indent (optional): The number of leading tab characters to be written

    Examples:
        >>> var = declare_variable(sys.stdout, "x")
        >>> return_variable(sys.stdout, var)
        return x;
    """
    write_indent(outfile, indent)
    outfile.write("return " + str(var) + ";\n")
